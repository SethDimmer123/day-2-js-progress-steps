...
prev.addEventListener('click',() =>{
    ...
    if(currentActive > 1) { // length means the amount of circles
        // if currentAtive is less than 1
...
function update() {
    circles.forEach(function(circle,index){
        if(index < currentActive) { // if the index of the particular 
            // circle is less than the current active.
...
else if(currentActive === circles.length) { // if the currentActive is at the end 
    // of the circles length
    // then i disable the next button.
    next.disabled = true
}
else{
    prev.disabled = false
